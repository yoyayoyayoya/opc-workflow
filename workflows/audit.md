---
description: Sprint 审计工作流 — 零信任审核上一个 Sprint 的代码质量（独立会话执行）
---

# Sprint Audit Workflow

当用户输入 `/audit` 时触发此工作流。

## 核心原则

1. **零信任**：假设上一个 Sprint 的所有代码和测试都可能是假的
2. **只审不写**：本 workflow 不修改任何业务代码，只产出审计报告
3. **独立会话**：必须在与开发 Sprint 不同的会话中执行
4. **证据驱动**：所有结论必须附带具体文件路径和行号

> [!IMPORTANT]
> **🐍 虚拟环境强制要求（每一条 Python/pytest 命令都必须遵守）**
> 所有 Python 相关终端命令必须使用项目 venv，绝对禁止使用系统 Python 或 `uv run`。
> 标准执行方式：`.venv/bin/python -m pytest ...`
> 或先激活环境：`source .venv/bin/activate && pytest ...`
> 违反此规则 = 命令必然报错，审计流程中断。

---

## Step 1: 加载上下文（自动执行）

// turbo

读取以下文件：
```
1. docs_v2/sprint_tracker.md              → 最近完成的 Sprint 编号
2. docs_v2/sprints/sprint-{N}-review.md   → Sprint 声称做了什么
3. docs_v2/architecture.md 相关章节       → 应该是什么样
```

向用户确认：
> "准备审计 Sprint {N}。Sprint 声称完成了 {n} 个 Task。开始审计？"

---

## Step 2: 测试质量审计

### 2.1 空断言扫描
// turbo
搜索所有测试文件中的可疑断言：
```bash
grep -rn "assert True\|assert.*is not None\|assert.*is None\|pass$" tests/ --include="*.py"
```

标记所有命中为 🔴 可疑。

### 2.2 过度 Mock 扫描
// turbo
搜索 mock/patch 使用：
```bash
grep -rn "@patch\|@mock\|MagicMock\|mocker\.patch" tests/ --include="*.py"
```

逐条检查：被 mock 的对象是否是被测逻辑本身？如果是 → 🔴 假测试。

### 2.3 测试覆盖率检查（强制门禁）

// turbo
```bash
# ⚠️ 必须用 venv，禁止用 uv run 或系统 python
.venv/bin/python -m pytest tests/ -q --tb=short --timeout=30 2>&1 | tee /tmp/audit_pytest_output.txt; echo "EXIT_CODE:$?"
```

> [!CAUTION]
> **强制门禁 — 以下条件任意一条不满足，审计必须立即中止，不得继续 Step 3+：**
> 1. 命令必须成功执行（能看到 `passed` / `failed` 字样）
> 2. 必须在审计报告中写入类似 `N passed, M failed` 的**实际数字**
> 3. 如果命令报错（找不到 pytest / venv 等），必须先修复环境，禁止跳过
> 4. 「代码看起来对」不等于「测试通过」——没有 pytest 输出就没有证据

对比 Sprint review 中声称的测试数量 vs 实际运行数量。不一致 → 🔴。

---

## Step 3: 变异测试（最致命的一招）

对 Sprint 中修改的每个核心函数：

1. **记录原始代码**
2. **故意改坏**（例如：`return True` → `return False`，删掉关键行）
3. **运行相关测试**
4. **判定**：
   - 测试变红 ✅ → 测试是真的
   - 测试仍然绿 🔴 → 假测试，测试根本没验证这个逻辑
5. **还原代码**

> ⚠️ 每次变异后必须还原。变异测试不应留下任何代码变更。

---

## Step 4: 逻辑完整性审计

### 4.1 对比设计文档
读 architecture.md 中对应模块的设计 → 检查代码是否真的实现了设计。

重点检查：
- 函数体是否有真实实现（不是 `pass`、`TODO`、`NotImplementedError`）
- import 是否真的被使用
- 参数是否真的被消费（不是接收了参数但忽略了）

### 4.2 功能回归检查
Sprint review 声称的每个功能点，手动追踪代码路径确认真实存在。

---

## Step 5: 产出审计报告

写入 `docs_v2/sprints/audit-sprint-{N}.md`：

```markdown
# Sprint {N} 审计报告

## 审计范围
Sprint {N}，{n} 个 Task，{m} 个文件变更

## 测试基线（必填，无此项审计报告无效）
```
{实际粘贴 pytest 输出最后 3 行，例如：}
1 failed, 456 passed, 61 deselected in 80.54s
```
> 如果此处为空或写「未运行」，本审计报告自动视为无效。

## 测试质量

### 空断言（共 {x} 个）
| 文件 | 行号 | 代码 | 严重度 |
|------|------|------|--------|
| ... | ... | ... | 🔴/🟡 |

### 过度 Mock（共 {x} 个）
| 文件 | 行号 | 被 mock 对象 | 是否 mock 了被测逻辑 |
|------|------|-------------|-------------------|
| ... | ... | ... | 🔴/✅ |

### 变异测试结果（共 {x} 个变异）
| 函数 | 变异方式 | 测试是否捕获 | 判定 |
|------|---------|-------------|------|
| ... | ... | ... | ✅/🔴 |

## 逻辑完整性

### 假实现（共 {x} 个）
| 文件 | 行号 | 问题描述 |
|------|------|---------|
| ... | ... | ... |

### 设计 vs 实现差距
| 设计要求 | 实现状态 | 差距 |
|---------|---------|------|
| ... | ... | ... |

## 总评
- 测试可信度：{高/中/低}
- 代码完整度：{高/中/低}
- 建议：{通过 / 需修复后重审}

## 修复清单（如有）
- [ ] {具体修复项 1}
- [ ] {具体修复项 2}
```

---

## Step 6: 汇报

向用户汇报：
> "Sprint {N} 审计完成。发现 {x} 个假测试、{y} 个假实现、{z} 个设计差距。建议：{通过/需修复}。详见 audit-sprint-{N}.md。"

**强制暂停。等待用户决策。**